...
export const airfetchLoader = async (src) => {
  ...
  if (src.inputs) {
    settings = {
      _apiEditorKey: process.env[src.settings['apiKeyName']] || src.settings['apiKeyName'],
      _baseId: process.env[src.settings['baseIdName']] || src.settings['baseIdName'],
    }
  }
  // content = await getContent(src.bases, false, true, inputs)
...
export const airfetchLoaderWorker = async (src, request) => {
  let results = []
  let settings

  if (src.settings) {
    settings = {
      _apiEditorKey: request.headers.get(src.settings['apiKeyName']) || src.settings['apiKeyName'],
      _baseId: request.headers.get(src.settings['baseIdName']) || src.settings['baseIdName'],
    }
  }
  // content = await getContent(src.bases, false, true, inputs)
